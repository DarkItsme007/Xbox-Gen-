...
print(Fore.BLUE+"[!] Select A Option")
print("")
print(Fore.CYAN+"[1] Xbox AutoPurchaser")
...
locale = (str(input(Fore.LIGHTMAGENTA_EX+"Locale:  ")))
proxies = input(Fore.LIGHTMAGENTA_EX+"Proxy ( Leave It Blank If No ): ")
fullBin = (str(input(Fore.LIGHTMAGENTA_EX+"BIN: ")))
...
postal_code =  input(Fore.LIGHTMAGENTA_EX+"Postal Code:")
...
"t": response.split('id="t" value="')[1].split('"')[0]} 
    ...
    if not vcc_req.ok:
        ...
        sprint(Fore.BLUE+f"[-] Email: "+email)
        sprint(Fore.GREEN+f"[-] VCC:"+vcc)
        sprint(vcc_req.text,"y")
        sprint("")
        Counter.Dead += 1
        ctypes.windll.kernel32.SetConsoleTitleW(
                f"Dark Cloud Xbox AutoPurchaser | discord.gg/darkcloud"
                f" | Hits: {Counter.Hit}"
        ...
        try:
            ...
            pass
        return
    sprint(Fore.LIGHTGREEN_EX+f"[+] Added card!","c")
    sprint(Fore.LIGHTGREEN_EX+f"[+] Email: {email}")
    sprint(Fore.LIGHTGREEN_EX+f"[+] VCC: {vcc}")
    Counter.Linked += 1
    ctypes.windll.kernel32.SetConsoleTitleW(
                f"Xbox AutoPurchaser - discord.gg/darkcloud"
                f" | Hits: {Counter.Hit}"
                f" - Linked: {Counter.Linked}"
                f" - Dead: {Counter.Dead}")
    ...
    json=json_data,
            ...
            return
    if not response.ok:
        sprint(Fore.RED+f"[-] Failed to purchase Gamepass! ")
        sprint(Fore.RED+f"[-] Email: {email}")
    ...
    try:
        ...
        sprint(response.text,"c")
        sprint(Fore.LIGHTMAGENTA_EX+f"[+] Purchased Gamepass! ")
        sprint(Fore.GREEN+f"Email: {email}")
        sprint(Fore.GREEN+f"VCC: {vcc}")
        Counter.Hit =+1
        ctypes.windll.kernel32.SetConsoleTitleW(
                f"Xbox AutoPurchaser - discord.gg/darkcloud"
        ...
        sprint(f"[-] Failed to purchase Gamepass! ")
        sprint(Fore.RED+f"[-] Email: {email}")
        sprint(Fore.RED+f"[-] VCC: {vcc}")
        Counter.Dead =+1 
        ctypes.windll.kernel32.SetConsoleTitleW(
...
print("")
print("")
thread_count  = int(input(Fore.BLUE+"Threads : "))
